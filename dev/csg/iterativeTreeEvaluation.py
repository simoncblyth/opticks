...
def postordereval_i1(node, debug=0):
    ...
    * loopR "3" -> "12 13 6 14 15 7 3"   
    ...
    while len(roots) > 0:
        ...
        while c < nn: 
            if nn >= 3 and c < nn - 2 and nodes[c+1].idx - nodes[c+0].idx == 1 and 2*nodes[c+2].idx == nodes[c+0].idx:
                ...
                if debug>2:
                    print "lrp (%d %d %d) " % (l.idx, r.idx, p.idx )
            else:
                ...
                if debug>2:
                    print "el,er,p (%s %s %d) " % (el, er, p.idx )
            pass
...
def postordereval_i2(root, debug=0): 
    ...
    * NB no looper support, so must skip this one in equality asserts
    ...
    while c >= 0: 
        ...
        if c > 2 and s[c-1].idx - s[c].idx == 1 and s[c-2].idx*2 == s[c].idx: 
            l = s[c-0]
            r = s[c-1]
            p = s[c-2]
...
if __name__ == '__main__':
    ...
    for root in roots:
        ...
        print " postorder:" + " ".join(map(lambda node:str(node.idx), nodes))
        print 
