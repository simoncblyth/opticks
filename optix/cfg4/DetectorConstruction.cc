...
G4Material* DetectorConstruction::MakeVacuum()
{
    G4double a, z, density;
    G4Material* m = new G4Material("Vacuum",z=1.,a=1.01*g/mole,
                          density=universe_mean_density,kStateGas,0.1*kelvin,
                          1.e-19*pascal);

    G4double en[] = {2.0*eV,7.0*eV,7.14*eV};
    G4double ri[]  =  {1.,1.,1.};
    assert(sizeof(ri) == sizeof(en));
    const G4int ne = sizeof(en)/sizeof(G4double);

    G4MaterialPropertiesTable* mpt = new G4MaterialPropertiesTable();
    mpt->AddProperty("RINDEX", en, ri, ne);
