...
if __name__ == '__main__':
    ...
    args = opticks_main(doc=__doc__, tag="1", src="torch", det="concentric", c2max=2.0, tagoffset=0,  dbgseqhis=0)
    log.info("tag %s src %s det %s c2max %s  " % (args.utag,args.src,args.det, args.c2max))

    cf = CF(tag=args.tag, src=args.src, det=args.det, dbgseqhis=args.dbgseqhis)
