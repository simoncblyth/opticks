...
{
    if(verbose)
    std::cout << "[ U4SolidTree::ApplyZCutTree zcut " << zcut << " original.GetName " << original->GetName() << std::endl ; 

    U4SolidTree* zs = new U4SolidTree(original); 
    zs->apply_cut( zcut );  
    //zs->dump("U4SolidTree::ApplyZCutTree"); 

    if(verbose)
    std::cout << "] U4SolidTree::ApplyZCutTree  zs.root.GetName " << zs->root->GetName()  << std::endl ; 
    return zs->root ; 
}

void U4SolidTree::Draw(const G4VSolid* original, const char* msg ) // static
{
    LOG(LEVEL) << "[" ; 
    if(original == nullptr )
    {
        std::cout << "ERROR U4SolidTree::Draw got nullptr original : msg " << msg << std::endl ; 
        return ; 
    }    
...
U4SolidTree::U4SolidTree(const G4VSolid* original_ ) 
    :
    original(original_),
...
void U4SolidTree::init()
{
    if(verbose) std::cout << "U4SolidTree::init" << std::endl ; 
    instrumentTree(); 
}

void U4SolidTree::dump(const char* msg) const 
{
    dumpTree(msg); 
    dumpUp(msg); 
}

void U4SolidTree::instrumentTree()
{
    if(verbose) std::cout << "U4SolidTree::instrumentTree parent " << std::endl ; 
    parent_map->clear(); 
    (*parent_map)[root] = nullptr ;  // root has no parent by definition
    parent_r( root, 0 ); 

    if(verbose) std::cout << "U4SolidTree::instrumentTree depth " << std::endl ; 
    depth_map->clear(); 
    depth_r(root, 0 ); 

    if(verbose) std::cout << "U4SolidTree::instrumentTree inorder " << std::endl ; 
    inorder->clear();
    inorder_r(root, 0 ); 

    if(verbose) std::cout << "U4SolidTree::instrumentTree rinorder " << std::endl ; 
    rinorder->clear();
    rinorder_r(root, 0 ); 

    if(verbose) std::cout << "U4SolidTree::instrumentTree preorder " << std::endl ; 
    preorder->clear();
    preorder_r(root, 0 ); 

    if(verbose) std::cout << "U4SolidTree::instrumentTree rpreorder " << std::endl ; 
    rpreorder->clear();
    rpreorder_r(root, 0 ); 

    if(verbose) std::cout << "U4SolidTree::instrumentTree postorder " << std::endl ; 
    postorder->clear(); 
    postorder_r(root, 0 ); 

    if(verbose) std::cout << "U4SolidTree::instrumentTree rpostorder " << std::endl ; 
    rpostorder->clear(); 
    rpostorder_r(root, 0 ); 

    if(verbose) std::cout << "U4SolidTree::instrumentTree names" << std::endl ; 
    names->clear(); 
    collectNames_inorder_r( root, 0 ); 

    if(verbose)
    {
        std::cout << "U4SolidTree::instrumentTree names.size " << names->size() << std::endl ; 
        for(unsigned i=0 ; i < names->size() ; i++) std::cout << "[" << (*names)[i] << "]" << std::endl ;  
    }
    if(verbose) std::cout << "U4SolidTree::instrumentTree nameprefix" << std::endl ; 
    nameprefix = CommonPrefix(names); 
    if(verbose) std::cout << "U4SolidTree::instrumentTree nameprefix [" << nameprefix << "]" << std::endl ; 

    if(verbose) std::cout << "U4SolidTree::instrumentTree [ original_num_node " << std::endl ; 
    int original_num_node = NumNode_r(original, 0); 
    if(verbose) std::cout << "U4SolidTree::instrumentTree ] original_num_node : " << original_num_node  << std::endl ; 

    if(verbose) std::cout << "U4SolidTree::instrumentTree [ root_num_node " << std::endl ; 
    int root_num_node = NumNode_r(root, 0); 
    if(verbose) std::cout << "U4SolidTree::instrumentTree ] root_num_node : " << root_num_node << std::endl ; 
    ...
    if(verbose) printf("U4SolidTree::instrumentTree width %d height %d \n", width, height );     
    canvas->resize( width+extra_width, height+extra_height );    
}


const char* U4SolidTree::CommonPrefix(const std::vector<std::string>* a) // static
{
    std::vector<std::string> aa(*a); 

    if(verbose)
    {
        std::cout << "U4SolidTree::CommonPrefix aa.size (before sort) " << aa.size() << std::endl ;  
    ...
    if(verbose)
    {
        std::cout << "U4SolidTree::CommonPrefix aa.size (after sort) " << aa.size() << std::endl ;  
        for(unsigned i=0 ; i < aa.size() ; i++) std::cout << "[" << aa[i] << "]" << std::endl ;  
    }
    ...
    if(verbose)
    {
        std::cout << "U4SolidTree::CommonPrefix s1 " << s1 << std::endl ;  
        std::cout << "U4SolidTree::CommonPrefix s2 " << s2 << std::endl ;  
        std::cout << "U4SolidTree::CommonPrefix prefix " << prefix << std::endl ;  
        std::cout << "U4SolidTree::CommonPrefix prefix_ " << prefix_ << std::endl ;  
    }
...
**/
void U4SolidTree::parent_r( const G4VSolid* node_, int depth)
{
...
void U4SolidTree::depth_r(const G4VSolid* node_, int depth)
{
    if( node_ == nullptr ) return ; 
    const G4VSolid* node = Moved(node_ ); 
...
void U4SolidTree::inorder_r(const G4VSolid* node_, int depth)
{
    if( node_ == nullptr ) return ; 
    const G4VSolid* node = Moved(node_ ); 
...
void U4SolidTree::rinorder_r(const G4VSolid* node_, int depth)
{
    if( node_ == nullptr ) return ; 
    const G4VSolid* node = Moved(node_ ); 
...
void U4SolidTree::preorder_r(const G4VSolid* node_, int depth)
{
    if( node_ == nullptr ) return ; 
    const G4VSolid* node = Moved(node_ ); 
...
void U4SolidTree::rpreorder_r(const G4VSolid* node_, int depth)
{
    if( node_ == nullptr ) return ; 
    const G4VSolid* node = Moved(node_ ); 
...
void U4SolidTree::postorder_r(const G4VSolid* node_, int depth)
{
    if( node_ == nullptr ) return ; 
    const G4VSolid* node = Moved(node_ ); 
...
void U4SolidTree::rpostorder_r(const G4VSolid* node_, int depth)
{
    if( node_ == nullptr ) return ; 
    const G4VSolid* node = Moved(node_ ); 
...
const G4VSolid* U4SolidTree::parent(  const G4VSolid* node ) const { return parent_map->count(node) == 1 ? (*parent_map)[node] : nullptr ; }
G4VSolid*       U4SolidTree::parent_( const G4VSolid* node ) const { const G4VSolid* p = parent(node); return const_cast<G4VSolid*>(p) ; }

int U4SolidTree::depth( const G4VSolid* node_) const { return (*depth_map)[node_] ; }
int U4SolidTree::in(    const G4VSolid* node_) const { return (*in_map)[node_] ; }
int U4SolidTree::rin(   const G4VSolid* node_) const { return (*rin_map)[node_] ; }
int U4SolidTree::pre(   const G4VSolid* node_) const { return (*pre_map)[node_] ; }
int U4SolidTree::rpre(  const G4VSolid* node_) const { return (*rpre_map)[node_] ; }
int U4SolidTree::post(  const G4VSolid* node_) const { return (*post_map)[node_] ; }
int U4SolidTree::rpost( const G4VSolid* node_) const { return (*rpost_map)[node_] ; }
...
int U4SolidTree::index( const G4VSolid* n, int mode ) const  
{
    int idx = -1 ; 
    switch(mode)
    {
...
const char* U4SolidTree::IN_ = "IN" ; 
const char* U4SolidTree::RIN_ = "RIN" ; 
const char* U4SolidTree::PRE_ = "PRE" ; 
const char* U4SolidTree::RPRE_ = "RPRE" ; 
const char* U4SolidTree::POST_ = "POST" ; 
const char* U4SolidTree::RPOST_ = "RPOST" ; 

const char* U4SolidTree::OrderName(int mode) // static
{
...
int U4SolidTree::zcls( const G4VSolid* node_ ) const 
{ 
    return zcls_map->count(node_) == 1 ? (*zcls_map)[node_] : UNDEFINED ; 
}

void U4SolidTree::set_zcls( const G4VSolid* node_, int zc )
{
    (*zcls_map)[node_] = zc  ;
} 

char U4SolidTree::mkr( const G4VSolid* node_) const 
{ 
    return mkr_map->count(node_) == 1 ? (*mkr_map)[node_] : ' ' ; 
}

void U4SolidTree::set_mkr( const G4VSolid* node_, char mk )
{
    (*mkr_map)[node_] = mk  ;
} 


bool U4SolidTree::is_include( const G4VSolid* node_) const 
{
    return zcls(node_) == INCLUDE ; 
}
bool U4SolidTree::is_exclude( const G4VSolid* node_) const 
{
    return zcls(node_) == EXCLUDE ; 
}

bool U4SolidTree::is_exclude_include( const G4VSolid* node_) const 
{
...
bool U4SolidTree::is_include_exclude( const G4VSolid* node_) const 
{
    if(!IsBoolean(node_)) return false ; 
    const G4VSolid* left_  = Left(node_); 
    const G4VSolid* right_ = Right(node_); 
...
bool U4SolidTree::is_crux( const G4VSolid* node_ ) const 
{
    if(!IsBoolean(node_)) return false ; 
    const G4VSolid* left_  = Left(node_); 
    const G4VSolid* right_ = Right(node_); 
...
int U4SolidTree::num_prim_r(const G4VSolid* node_) const
{   
    if(!node_) return 0 ; 
...
int U4SolidTree::num_prim() const 
{
    return num_prim_r(root); 
}

int U4SolidTree::num_node() const
{
    return NumNode_r(root, 0) ; 
}

/**
U4SolidTree::NumNode_r
------------------
...
int U4SolidTree::NumNode_r(const G4VSolid* node_, int depth) // static 
{
    int num = node_ ? 1 : 0 ;
    if( node_ )
    {
...
int U4SolidTree::num_node_select(int qcls) const
{
    return num_node_select_r(root, qcls) ; 
}
int U4SolidTree::num_node_select_r(const G4VSolid* node_, int qcls) const 
{
    int zcl = zcls(node_) ;
    const G4VSolid* node = Moved(node_ ); 
...
const char* U4SolidTree::desc() const 
{
...
void U4SolidTree::prune(bool act, int pass)
{
    int num_include = num_node_select(INCLUDE) ;

    if(verbose)
    printf("U4SolidTree::prune act:%d pass:%d num_include %d \n", act, pass, num_include);

    if( num_include == 0)
    {
        if(verbose)
        printf("U4SolidTree::prune act:%d pass:%d find zero remaining nodes : num_include %d, will set root to nullptr \n", act, pass, num_include);

        if(act)
        {
            if(verbose)
            printf("U4SolidTree:::prune act:%d pass:%d setting root to nullptr \n", act, pass);
...
/**
U4SolidTree::prune_crux
-------------------

**/

void U4SolidTree::prune_crux(G4VSolid* x, bool act, int pass)
{
    assert( x );
    ...
    {
        if(verbose) 
        printf("U4SolidTree::prune_crux act:%d pass:%d parent disqualified as not INCLUDE : handle as root of the new tree\n", act, pass) ; 
    ...
    if( p != nullptr )   // non-root prune
    {
        ...
        if(x_is_p_left)
        {
            ...
            {
                if(verbose)
                printf("U4SolidTree:::prune_crux act:%d pass:%d SetLeft changing p.left to survivor \n", act, pass);
                SetLeft(p, survivor) ; 
                edited = true ; 
            }
        }
        else if(x_is_p_right)
        {
            ...
            {
                if(verbose)
                printf("U4SolidTree:prune_crux act:%d pass:%d SetRight changing p.right to survivor  \n", act, pass);
                SetRight(p, survivor) ; 
                edited = true ; 
            }
        }
    ...
    else           // root prune
    {
        if( act )
        {
            ...
            if(verbose)
            printf("U4SolidTree::prune_crux act:%d pass:%d changing root %s to survivor %s \n", act, pass, root_name.c_str(), survivor_name.c_str() );
            root = survivor ;
            edited = true ; 
        }
    }
...
void U4SolidTree::draw(const char* msg, int pass) 
{
    canvas->clear();
...
void U4SolidTree::dumpNames(const char* msg) const 
{
...
void U4SolidTree::zdump(const char* msg) const 
{
    std::cout << msg << std::endl ; 
    int mode = IN ; 
    zdump_r(root, mode); 
...
void U4SolidTree::zdump_r( const G4VSolid* node_, int mode ) const 
{
    if( node_ == nullptr ) return ;
    ...
    bool can_z = U4SolidTree::CanZ(node) ;
...
void U4SolidTree::draw_r( const G4VSolid* node_, int mode )
{
    if( node_ == nullptr ) return ;
    ...
    bool can_y = U4SolidTree::CanY(node) ;
    bool can_z = U4SolidTree::CanZ(node) ;
    if(verbose) std::cout << "U4SolidTree::draw_r can_z " << can_z << std::endl ;  
...
const char* U4SolidTree::EntityTypeName(const G4VSolid* solid)   // static
{
    G4GeometryType type = solid->GetEntityType();  // G4GeometryType typedef for G4String
    return strdup(type.c_str()); 
}

const char* U4SolidTree::EntityTag(const G4VSolid* node_, bool move)   // static
{
...
const char* U4SolidTree::G4Torus_              = "Tor" ;
const char* U4SolidTree::G4Box_                = "Box" ;
const char* U4SolidTree::G4Orb_                = "Orb" ;
const char* U4SolidTree::G4MultiUnion_         = "MUN" ;
const char* U4SolidTree::G4Sphere_             = "Sph" ;

const char* U4SolidTree::G4UnionSolid_         = "Uni" ;
const char* U4SolidTree::G4SubtractionSolid_   = "Sub" ;
...
const char* U4SolidTree::EntityTag_( const G4VSolid* solid )
{
    ...
    switch(etype)
    {
       ...
       case _G4Sphere:            s = G4Sphere_            ; break ; 
...
const char* U4SolidTree::DirtyEntityTag_( const G4VSolid* node )
{
...
int U4SolidTree::EntityType(const G4VSolid* solid)   // static 
{
...
std::string U4SolidTree::Desc(const G4VSolid* solid) // static
{
    std::stringstream ss ; 
...
bool U4SolidTree::IsBoolean(const G4VSolid* solid) // static
{
    return dynamic_cast<const G4BooleanSolid*>(solid) != nullptr ; 
}
bool U4SolidTree::IsDisplaced(const G4VSolid* solid) // static
{
    return dynamic_cast<const G4DisplacedSolid*>(solid) != nullptr ; 
}
const G4VSolid* U4SolidTree::Left(const G4VSolid* solid ) // static
{
    return IsBoolean(solid) ? solid->GetConstituentSolid(0) : nullptr ; 
}
const G4VSolid* U4SolidTree::Right(const G4VSolid* solid ) // static
{
    return IsBoolean(solid) ? solid->GetConstituentSolid(1) : nullptr ; 
}
G4VSolid* U4SolidTree::Left_(G4VSolid* solid ) // static
{
    return IsBoolean(solid) ? solid->GetConstituentSolid(0) : nullptr ; 
}
G4VSolid* U4SolidTree::Right_(G4VSolid* solid ) // static
{
    return IsBoolean(solid) ? solid->GetConstituentSolid(1) : nullptr ; 
}
...
**/
const G4VSolid* U4SolidTree::Moved( G4RotationMatrix* rot, G4ThreeVector* tla, const G4VSolid* node )  // static
{
...
const G4VSolid* U4SolidTree::Moved( const G4VSolid* node )  // static
{
    const G4DisplacedSolid* disp = dynamic_cast<const G4DisplacedSolid*>(node) ; 
    return disp ? disp->GetConstituentMovedSolid() : node  ;
}

G4VSolid* U4SolidTree::Moved_( G4RotationMatrix* rot, G4ThreeVector* tla, G4VSolid* node )  // static
{
...
G4VSolid* U4SolidTree::Moved_( G4VSolid* node )  // static
{
    G4DisplacedSolid* disp = dynamic_cast<G4DisplacedSolid*>(node) ; 
    return disp ? disp->GetConstituentMovedSolid() : node  ;
}
...
int U4SolidTree::maxdepth() const  
{
    return Maxdepth_r( root, 0 );  
}

int U4SolidTree::Maxdepth_r( const G4VSolid* node_, int depth)  // static 
{
    return IsBoolean(node_) ? std::max( Maxdepth_r(U4SolidTree::Left(node_), depth+1), Maxdepth_r(U4SolidTree::Right(node_), depth+1)) : depth ; 
}

/**
U4SolidTree::dumpTree
------------------
 
Postorder traversal of CSG tree
**/
void U4SolidTree::dumpTree(const char* msg ) const 
{
    std::cout << msg << " maxdepth(aka height) " << height << std::endl ; 
    dumpTree_r(root, 0 ); 
}

void U4SolidTree::dumpTree_r( const G4VSolid* node_, int depth  ) const
{
    if(IsBoolean(node_))
    {
        dumpTree_r(U4SolidTree::Left(node_) , depth+1) ; 
        dumpTree_r(U4SolidTree::Right(node_), depth+1) ; 
    }
...
int U4SolidTree::classifyTree(double zcut)  
{
    crux->clear(); 
    if(verbose) std::cout << "U4SolidTree::classifyTree against zcut " << zcut  << std::endl ; 
    int zc = classifyTree_r(root, 0, zcut); 
    return zc ; 
}


int U4SolidTree::classifyTree_r(G4VSolid* node_, int depth, double zcut )
{
    int zcl = 0 ; 
...
int U4SolidTree::classifyMask(const G4VSolid* top) const   // NOT USED ?
{
    return classifyMask_r(top, 0); 
}
int U4SolidTree::classifyMask_r( const G4VSolid* node_, int depth ) const 
{
    int mask = 0 ; 
    if(IsBoolean(node_))
    {
...
int U4SolidTree::ClassifyZCut( double az0, double az1, double zcut ) // static
{
    assert( az1 > az0 ); 
    int cls = UNDEFINED ; 
...
const char* U4SolidTree::UNDEFINED_ = "UNDEFINED" ; 
const char* U4SolidTree::INCLUDE_   = "INCLUDE" ; 
const char* U4SolidTree::STRADDLE_  = "STRADDLE" ; 
const char* U4SolidTree::EXCLUDE_   = "EXCLUDE" ; 

const char* U4SolidTree::ClassifyName( int zcl ) // static 
{
    const char* s = nullptr ; 
    switch( zcl )
    {
...
const char* U4SolidTree::ClassifyMaskName( int zcl ) // static
{
    std::stringstream ss ; 
...
void U4SolidTree::apply_cut(double zcut)
{
    if(verbose)
    printf("U4SolidTree::apply_cut %7.2f \n", zcut );

    if(verbose)
    Draw(root, "U4SolidTree::apply_cut root before cut"); 
    ...
    while( root != nullptr && zcls(root) != INCLUDE && pass < maxpass )
    {
        ...
        if(verbose)
        draw("U4SolidTree::apply_cut before prune", pass );
...
void U4SolidTree::collectNames_inorder_r( const G4VSolid* n_, int depth )
{
...
void U4SolidTree::cutTree_r( const G4VSolid* node_, int depth, double zcut )
{
    if(IsBoolean(node_))
    {
        cutTree_r( U4SolidTree::Left(node_) , depth+1, zcut ) ; 
        cutTree_r( U4SolidTree::Right(node_), depth+1, zcut ) ; 
    }
    else
    {
...
void U4SolidTree::collectNodes( std::vector<const G4VSolid*>& nodes, const G4VSolid* top, int query_zcls  )
{
    collectNodes_r(nodes, top, 0, query_zcls);  
}

void U4SolidTree::collectNodes_r( std::vector<const G4VSolid*>& nodes, const G4VSolid* node_, int query_zcls, int depth  )
{
    if(IsBoolean(node_))
    {
...
void U4SolidTree::ApplyZCut( G4VSolid* node_, double local_zcut ) // static
{
    G4VSolid* node = Moved_(node_ ); 
    if(verbose) std::cout << "U4SolidTree::ApplyZCut " << EntityTypeName(node) << std::endl ; 
    switch(EntityType(node))
    {
        ...
        { 
            std::cout 
                << "U4SolidTree::ApplyZCut FATAL : not implemented for entityType " 
                << EntityTypeName(node) 
                << std::endl 
                ; 
            assert(0) ; 
...
/**
U4SolidTree::ApplyZCut_G4Ellipsoid
--------------------------------
...
void U4SolidTree::ApplyZCut_G4Ellipsoid( G4VSolid* node, double local_zcut)
{  
...
/**
U4SolidTree::ApplyZCut_G4Polycone_NotWorking
-----------------------------------------
...
void U4SolidTree::ApplyZCut_G4Polycone_NotWorking( G4VSolid* node, double local_zcut)
{  
...
/**
U4SolidTree::ApplyZCut_G4Polycone
------------------------------
...
void U4SolidTree::ApplyZCut_G4Polycone( G4VSolid* node, double local_zcut)
{  
    G4Polycone* polycone = dynamic_cast<G4Polycone*>(node) ;  
    assert(polycone); 
...
void U4SolidTree::ApplyZCut_G4Tubs( G4VSolid* node_ , double local_zcut )
{ 
...
void U4SolidTree::dumpUp(const char* msg) const 
{
    assert( parent_map ); 
    std::cout << msg << std::endl ; 
    dumpUp_r(root, 0); 
...
void U4SolidTree::dumpUp_r(const G4VSolid* node, int depth) const  
{
    if(IsBoolean(node))
    {
        dumpUp_r(U4SolidTree::Left(  node ), depth+1 ); 
        dumpUp_r(U4SolidTree::Right( node ), depth+1 ); 
    }
    else
    {
...
void U4SolidTree::getTreeTransform( G4RotationMatrix* rot, G4ThreeVector* tla, const G4VSolid* node ) const 
{
...
G4VSolid* U4SolidTree::DeepClone( const  G4VSolid* solid )  // static 
{
    LOG(LEVEL) << "[" ; 
...
G4VSolid* U4SolidTree::DeepClone_r( const G4VSolid* node_, int depth, G4RotationMatrix* rot, G4ThreeVector* tla )  // static 
{
    ...
    ExpectNonNull( clone, "U4SolidTree::DeepClone_r GOT null clone " ); 
    return clone ; 
}    

/**
U4SolidTree::BooleanClone
-----------------------
...
G4VSolid* U4SolidTree::BooleanClone( const  G4VSolid* solid, int depth, G4RotationMatrix* rot, G4ThreeVector* tla ) // static
{
    if(verbose) std::cout << "U4SolidTree::BooleanClone" << std::endl ; 

    ExpectNoRotation(    rot, "U4SolidTree::BooleanClone expect_rot ERROR " ); 
    ExpectNoTranslation( tla, "U4SolidTree::BooleanClone expect_tla ERROR (not expecting more than one level of translation) ", true ); 
    ...
    ExpectNoDisplaced(     left, "U4SolidTree::BooleanClone expect_left ERROR "); 
    ExpectNoRotation(     &lrot, "U4SolidTree::BooleanClone expect_lrot ERROR : G4 should never have left transforms  " ); 
    ExpectNoTranslation(  &ltra, "U4SolidTree::BooleanClone expect_ltra ERROR : not expectsing left translation", false ); 
    ExpectNoRotation(     &rrot, "U4SolidTree::BooleanClone expect_rrot ERROR : simplifying assumptions  " ); 
...
void U4SolidTree::DeepCloneDump( int depth, const G4VSolid* node_ , const G4VSolid* node , G4ThreeVector* tla )
{
    LOG(LEVEL) 
        << "[ depth " << depth 
    ...
    {
        std::cout 
            << "U4SolidTree::DeepClone_r(preorder visit)"
            << " type " << std::setw(20) << EntityTypeName(node)
            << " name " << std::setw(20) << node->GetName()
            << " depth " << std::setw(2) << depth
            ; 
...
void U4SolidTree::ExpectNoRotation( const G4RotationMatrix* rot, const char* msg )
{
...
void U4SolidTree::ExpectNoTranslation( const G4ThreeVector* tla, const char* msg, bool skip_assert )
{
    if(!tla) return ; 
...
void U4SolidTree::ExpectNoDisplaced( const G4VSolid* solid, const char* msg )
{
...
void U4SolidTree::ExpectNonNull( const G4VSolid* clone, const char* msg )
{
...
void U4SolidTree::CheckBooleanClone( const G4VSolid* clone, const G4VSolid* left, const G4VSolid* right ) // static
{
    bool expect ; 

    if(!clone) std::cout << "U4SolidTree::CheckBooleanClone FATAL " << std::endl ; 
...
void U4SolidTree::GetBooleanBytes(char** bytes, int& num_bytes, const G4VSolid* solid ) // static
{
...
int U4SolidTree::CompareBytes( char* bytes0, char* bytes1, int num_bytes ) // static
{
    int mismatch = 0 ; 
...
void U4SolidTree::PlacementNewDupe( G4VSolid* solid) // static
{
...
void U4SolidTree::SetRight(  G4VSolid* node, G4VSolid* right, G4RotationMatrix* rrot, G4ThreeVector* rtla )
{
...
void U4SolidTree::SetLeft(  G4VSolid* node, G4VSolid* left)  // static 
{
    assert( dynamic_cast<G4DisplacedSolid*>(node) == nullptr ) ; 
...
G4VSolid* U4SolidTree::PrimitiveClone( const  G4VSolid* solid )  // static 
{
    ...
    else if( type == _G4Sphere )
    {
        const G4Sphere* sph = dynamic_cast<const G4Sphere*>(solid) ; 
        clone = new G4Sphere(*sph) ;  
    }
    else
    {
        std::cout 
            << "U4SolidTree::PrimitiveClone FATAL unimplemented prim type " << EntityTypeName(solid) 
            << std::endl 
            ;
        assert(0); 
    } 
...
G4VSolid* U4SolidTree::PrimitiveClone( const  G4VSolid* solid, const char* name )  // static 
{
...
//const bool U4SolidTree::PROMOTE_TUBS_TO_POLYCONE = true ; 
const bool U4SolidTree::PROMOTE_TUBS_TO_POLYCONE = false ; 

G4VSolid* U4SolidTree::PromoteTubsToPolycone( const G4VSolid* solid ) // static
{
    const G4Tubs* tubs = dynamic_cast<const G4Tubs*>(solid) ; 
    assert(tubs); 
...
double U4SolidTree::getX( const G4VSolid* node ) const
{
    G4RotationMatrix* tree_rot = nullptr ; 
    G4ThreeVector    tree_tla(0., 0., 0. ); 
    getTreeTransform(tree_rot, &tree_tla, node ); 
...
double U4SolidTree::getY( const G4VSolid* node ) const
{
    G4RotationMatrix* tree_rot = nullptr ; 
    G4ThreeVector    tree_tla(0., 0., 0. ); 
    getTreeTransform(tree_rot, &tree_tla, node ); 
...
double U4SolidTree::getZ( const G4VSolid* node ) const
{
    G4RotationMatrix* tree_rot = nullptr ; 
    G4ThreeVector    tree_tla(0., 0., 0. ); 
    getTreeTransform(tree_rot, &tree_tla, node ); 
...
bool U4SolidTree::CanX(  const G4VSolid* solid ) 
{
    int type = EntityType(solid) ; 
    return type == _G4Box ; 
}
bool U4SolidTree::CanY(  const G4VSolid* solid ) 
{
    int type = EntityType(solid) ; 
    return type == _G4Box ; 
}
bool U4SolidTree::CanZ( const G4VSolid* solid ) // static
{
    int type = EntityType(solid) ; 
...
void U4SolidTree::XRange( double& x0, double& x1, const G4VSolid* solid) // static  
{
    switch(EntityType(solid))
    {
        case _G4Box:       GetXRange( dynamic_cast<const G4Box*>(solid),       x0, x1 );  break ; 
        case _G4Other:    { std::cout << "U4SolidTree::GetX FATAL : not implemented for entityType " << EntityTypeName(solid) << std::endl ; assert(0) ; } ; break ;  
    }
}
void U4SolidTree::YRange( double& y0, double& y1, const G4VSolid* solid) // static  
{
    switch(EntityType(solid))
    {
        case _G4Box:       GetYRange( dynamic_cast<const G4Box*>(solid),       y0, y1 );  break ; 
        case _G4Other:    { std::cout << "U4SolidTree::GetY FATAL : not implemented for entityType " << EntityTypeName(solid) << std::endl ; assert(0) ; } ; break ;  
    }
}
void U4SolidTree::ZRange( double& z0, double& z1, const G4VSolid* solid) // static  
{
    switch(EntityType(solid))
    {
        ...
        case _G4Other:    { std::cout << "U4SolidTree::GetZ FATAL : not implemented for entityType " << EntityTypeName(solid) << std::endl ; assert(0) ; } ; break ;  
    }
}



void U4SolidTree::GetXRange( const G4Box* const box, double& _x0, double& _x1 )  // static 
{
    _x1 = box->GetXHalfLength() ; 
    _x0 = -_x1 ; 
    assert( _x1 > 0. ); 
}
void U4SolidTree::GetYRange( const G4Box* const box, double& _y0, double& _y1 )  // static 
{
    _y1 = box->GetYHalfLength() ; 
    _y0 = -_y1 ; 
    assert( _y1 > 0. ); 
}
void U4SolidTree::GetZRange( const G4Box* const box, double& _z0, double& _z1 )  // static 
{
...
void U4SolidTree::GetZRange( const G4Ellipsoid* const ellipsoid, double& _z0, double& _z1 )  // static 
{
    _z1 = ellipsoid->GetZTopCut() ; 
    _z0 = ellipsoid->GetZBottomCut() ;  
}
void U4SolidTree::GetZRange( const G4Tubs* const tubs, double& _z0, double& _z1 )  // static 
{
    _z1 = tubs->GetZHalfLength() ;  
    _z0 = -_z1 ;  
    assert( _z1 > 0. ); 
}
void U4SolidTree::GetZRange( const G4Polycone* const polycone, double& _z0, double& _z1 )  // static 
{
    G4PolyconeHistorical* pars = polycone->GetOriginalParameters(); 
    unsigned num_z = pars->Num_z_planes ; 
    for(unsigned i=1 ; i < num_z ; i++)
...
void U4SolidTree::GetZRange( const G4Torus* const torus, double& _z0, double& _z1 )  // static 
{
