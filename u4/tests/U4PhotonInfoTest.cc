...
void test_GetRef()
{
    ...
    spho* p2 = U4PhotonInfo::GetRef(ctrack); 

    assert( p2->isIdentical(p0) ); 

    std::cout << " p2 " << p2->desc() << " p2 from U4PhotonInfo::GetRef(ctrack) " << std::endl ; 

    p2->set_gen(10); 
    p2->set_flg(101); 
    std::cout << " p2 " << p2->desc() << " after p2->set_flg(101) p2->set_gen(10) " << std::endl ; 

    spho p3 = U4PhotonInfo::Get(ctrack); 
    std::cout << " p3 " << p3.desc() << " U4PhotonInfo::Get(ctrack)  by value reflects the change made using the pointer " << std::endl ; 
