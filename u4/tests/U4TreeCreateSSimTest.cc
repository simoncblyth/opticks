...
U4TreeCreateSSimTest::U4TreeCreateSSimTest(const G4VPhysicalVolume* world )
    ...
    q_node( q_lvid > -1 ? st->pick_lvid_ordinal_node(q_lvid, q_lvid_ordinal, 'N' ) : nullptr ),
...
int U4TreeCreateSSimTest::get_combined_transform() const
{
    ...
    for(int j=-1 ; j < q_nd_num_child ; j++)
    {
        ...
        st->get_combined_transform(t,v,*q_node,n, out, t_stack ) ;
...
int U4TreeCreateSSimTest::get_combined_tran_and_aabb() const
{
    ...
    for(int j=-1 ; j < q_nd_num_child ; j++)
    {
        ...
        const Tran<double>* tran = leaf ? st->get_combined_tran_and_aabb(aabb,*q_node, nd, out, t_stack ) : nullptr ;
