...
(Exit, Exit) -> Return Closer Exit
    Both isect classified as Exit means origin is inside both,
    hence the closer isect is needed as it is the nature of
    CSG_INTERSECTION to pick the shape formed from the overlap
    of the constituents
...
* assume that find a way to special case reclassify "B Miss" into "B Exit"
  ...
  * rationalize the signalling (note that are re-using isect when there is miss
    and hence no intersection to pass info up the tree)
  ...
  * how to check are inside an infinite phicut ?  check (o.x,o.y) ? signed distances ?
  * think about a very thin wedge phicut and pure-z (eg orthographic view looking directly
    down on the cake) most of the time will miss the phicut hence do not want to
    promote that miss : as the phicut is effectively not there.
