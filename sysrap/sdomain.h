...
struct sdomain
{
    ...
    static NP* ConvertWavelengthToEnergy(const NP* prop, bool reverse=true );
...
inline NP* sdomain::ConvertWavelengthToEnergy(const NP* prop, bool reverse )
{
    assert( prop->uifc == 'f' && prop->ebyte == 8 );
    const double* pp = prop->cvalues<double>();

    NP* a = NP::MakeLike(prop);
    double* aa = a->values<double>();

    size_t ni = prop->shape[0];
    size_t nj = prop->shape[1];
    assert( nj == 2 );

    for(size_t _i = 0 ; _i < ni ; _i++ )
    {
        size_t i = reverse ? ni - 1 - _i : _i ;
        double nm = pp[2*i+0] ;
        double ev = hc_eVnm / nm ;

        double value = pp[2*i+1] ;
