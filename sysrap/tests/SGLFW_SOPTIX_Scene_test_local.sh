#!/bin/bash
usage(){ cat << EOU
SGLFW_SOPTIX_Scene_test_local.sh : triangulated raytrace and rasterized visualization
=======================================================================================

This is a "local" non-CMake build variant of SGLFW_SOPTIX_Scene_test.sh 
for development only. The binary created and this script are NOT 
included in release.   Use SGLFW_SOPTIX_Scene_test.sh for that. 
...
logging()
{
   ...
   export SOPTIX_Options__LEVEL=2
...
cptx=$OPTICKS_PREFIX/optix/objects-Debug/SysRap_OPTIX/SOPTIX.ptx
export SOPTIX__optixpath=$ptx
