...
With TEST=ALL the ~10 SSim::Load took ~20s when loading 
from slow /cvmfs which made this the slowest test. 
So avoid leaving TEST=ALL as the default to 
bring below 2.5s even with /cvmfs. 
...
int SSimTest::Main()
{
    bool ALL = strcmp(TEST, "findName") == 0 ;
    int rc = 0 ;
