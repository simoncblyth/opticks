...
struct srngcpu
{
    ...
    template<typename T>
    NP* sample_icdf(const NP* icdf_prop, size_t num_sample );
...
template <> inline double srngcpu::generate(){ return generate_double() ; }
template <> inline float srngcpu::generate(){ return generate_float() ; }
...
{
    NP* sample = NP::Make<T>(num_sample);
    T* ss = sample->values<T>();
    for(size_t i=0 ; i < sample->shape[0] ; i++)
    {
        T u = generate<T>();
        ss[i] = icdf_prop->interp(u);
    }
    return sample ;
}
