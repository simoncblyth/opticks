...
template<typename S>
inline NP* NP::ArrayFromDiscoMap( const std::map<int, S>& m )
{
    return nullptr ;    
}
template<>
inline NP* NP::ArrayFromDiscoMap( const std::map<int,int>& m )
{
    int ni = m.size() ; 
    int nj = 2 ; 
...
template<typename S>
inline void NP::DiscoMapFromArray( std::map<int, S>& m, const NP* a ){}

template<>
inline void NP::DiscoMapFromArray( std::map<int, int>& m, const NP* a )
{
...
template<typename S>
inline std::string NP::DescDiscoMap( const std::map<int, S>& m )
{
    std::stringstream ss ; 
    ss << "NP::DescDiscoMap" << std::endl << " m.size " << m.size() ; 
    std::string s = ss.str();    
    return s ; 
}

template<>
inline std::string NP::DescDiscoMap( const std::map<int,int>& m )
{
    int ni = m.size() ; 
    typedef std::map<int,int> MII ; 
    MII::const_iterator it = m.begin(); 
    std::stringstream ss ; 
    ss << "NP::DescDiscoMap" << std::endl << " m.size " << ni << std::endl ; 
    for(int i=0 ; i < ni ; i++)
    {
