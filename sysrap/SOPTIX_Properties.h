...
struct SOPTIX_Properties
{
    ...
    unsigned shaderExecutionReordering ;
...
inline SOPTIX_Properties::SOPTIX_Properties(OptixDeviceContext context)
{
    ...
    OPTIX_CHECK( optixDeviceContextGetProperty(context, OPTIX_DEVICE_PROPERTY_SHADER_EXECUTION_REORDERING             , &shaderExecutionReordering          , sizeof(unsigned int)) );
#else
    shaderExecutionReordering = 0; // Not supported on OptiX < 7.6.0
#endif
...
inline std::string SOPTIX_Properties::desc() const
{
    ...
    ss
        ...
        << std::endl
        << std::setw(40) << "shaderExecutionReordering"
        << " : "
        << std::setw(10) <<  shaderExecutionReordering
        << std::endl
        ;
