...
struct SYSRAP_API s_tv
{
    ...
    std::string desc_full0() const ;  // glm::to_string formatted
    std::string desc_full() const ;
...
inline std::string s_tv::desc_full0() const
{
    std::stringstream ss ;
    ss
        << "t " << glm::to_string(t)
...
inline std::string s_tv::desc_full() const
{
    ...
    const double* aa = glm::value_ptr(t);
    const double* bb = glm::value_ptr(v);
    const double* cc = glm::value_ptr(tv);
    ...
    glm::tvec4<double> mn = {-100,-100,-100, 1} ;
    glm::tvec4<double> mx = { 100, 100, 100, 1} ;

    glm::tvec4<double> t_mn = t * mn ;
    glm::tvec4<double> t_mx = t * mx ;

    const double* _mn   = glm::value_ptr(mn);
    const double* _mx   = glm::value_ptr(mx);
    const double* _t_mn = glm::value_ptr(t_mn);
    const double* _t_mx = glm::value_ptr(t_mx);

    ss << " mn   " ;
    for(int j=0 ; j < 4 ; j++) ss << " " << std::fixed << std::setw(12) << std::setprecision(4) << _mn[j] ;
    ss << "\n" ;

    ss << " mx   " ;
    for(int j=0 ; j < 4 ; j++) ss << " " << std::fixed << std::setw(12) << std::setprecision(4) << _mx[j] ;
    ss << "\n" ;

    ss << " t_mn " ;
    for(int j=0 ; j < 4 ; j++) ss << " " << std::fixed << std::setw(12) << std::setprecision(4) << _t_mn[j] ;
    ss << "\n" ;

    ss << " t_mx " ;
    for(int j=0 ; j < 4 ; j++) ss << " " << std::fixed << std::setw(12) << std::setprecision(4) << _t_mx[j] ;
    ss << "\n" ;
