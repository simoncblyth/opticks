...
void* OCtx_create_geometryinstance(void* geo_ptr, void* mat_ptr ) {                                                          return OCtx_()->create_geometryinstance(geo_ptr, mat_ptr); }
void* OCtx_create_geometrygroup(const std::vector<void*>& v_gi_ptr) {                                                        return OCtx_()->create_geometrygroup(v_gi_ptr); } 
void* OCtx_create_acceleration( const char* accel ) {                                                                        return OCtx_()->create_acceleration( accel ); } 

void  OCtx_set_geometrygroup_acceleration(void* gg_ptr, void* ac_ptr ) {       OCtx_()->set_geometrygroup_acceleration(gg_ptr, ac_ptr); }
void  OCtx_set_group_acceleration(        void* group_ptr, void* ac_ptr ) {    OCtx_()->set_group_acceleration(group_ptr, ac_ptr); }
void  OCtx_set_geometrygroup_context_variable( const char* key, void* gg_ptr){ OCtx_()->set_geometrygroup_context_variable(key, gg_ptr); }
...
void  OCtx_launch_instrumented(unsigned entry_point_index, unsigned width, unsigned height, double& t_prelaunch, double& t_launch)
{
    OCtx_()->launch_instrumented(entry_point_index, width, height, t_prelaunch, t_launch); 
}
...
void  OCtx_set_context_float4(const char* key, float x, float y, float z, float w ) { OCtx_()->set_context_float4(key, x, y, z, w); }
void  OCtx_set_context_int4(const char* key, int x, int y, int z, int w ) {           OCtx_()->set_context_int4(  key, x, y, z, w); }
void  OCtx_set_context_viewpoint( const glm::vec3& eye, const glm::vec3& U,  const glm::vec3& V, const glm::vec3& W, const float scene_epsilon ) 
{ 
  OCtx_()->set_context_viewpoint( eye, U, V, W, scene_epsilon ); 
}

void* OCtx_create_transform( bool transpose, const float* m44, const float* inverse_m44 ){ return OCtx_()->create_transform(transpose, m44, inverse_m44) ; }   
