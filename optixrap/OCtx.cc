...
struct OCtx 
{
    ...
    void test_recover_context()
    {
        optix::Context context_ = optix::Context::take((RTcontext)get()) ;  
    }
    ...
    void desc_buffer( void* ptr )
    {
        // recovering the buffer from the void* ptr 
        RTbuffer bufPtr = (RTbuffer)ptr ;  
...
void* OCtx_create_buffer(const NPYBase* arr, const char* key, char type, char flag)
{
    return OCtx_()->create_buffer(arr, key, type, flag);
}
void OCtx_desc_buffer(void* ptr)
{
    OCtx_()->desc_buffer(ptr);
}
unsigned OCtx_create_texture_sampler( void* buffer_ptr, const char* config )
{
    return OCtx_()->create_texture_sampler(buffer_ptr, config);
}
void OCtx_set_texture_param( void* buffer_ptr, unsigned tex_id, const char* param_key )
{
    OCtx_()->set_texture_param(buffer_ptr, tex_id, param_key);
}
void OCtx_download_buffer(NPYBase* arr, const char* key)
{
    OCtx_()->download_buffer(arr, key);
}
void OCtx_set_float4(const char* key, float x, float y, float z, float w )
{
    OCtx_()->set_float4(key, x, y, z, w);
}
void OCtx_set_int4(const char* key, int x, int y, int z, int w )
{
    OCtx_()->set_int4(key, x, y, z, w);
}
void OCtx_set_raygen_program( unsigned entry_point_index, const char* ptx_path, const char* func )
{
    OCtx_()->set_raygen_program(entry_point_index, ptx_path, func ); 
}
void OCtx_set_exception_program( unsigned entry_point_index, const char* ptx_path, const char* func )
{
    OCtx_()->set_exception_program(entry_point_index, ptx_path, func ); 
}
void OCtx_set_viewpoint( const glm::vec3& eye, const glm::vec3& U,  const glm::vec3& V, const glm::vec3& W, const float scene_epsilon )
{
    OCtx_()->set_viewpoint( eye, U, V, W, scene_epsilon ); 
}
void* OCtx_create_geometry(unsigned prim_count, const char* ptxpath, const char* bounds_func, const char* intersect_func )
{
    return OCtx_()->create_geometry(prim_count, ptxpath, bounds_func, intersect_func);  
    // resurrect from void* with : optix::Geometry geo = optix::Geometry::take((RTgeometry)geo_ptr);   
}
void* OCtx_create_material(const char* ptxpath, const char* closest_hit_func, unsigned entry_point_index )
{
    return OCtx_()->create_material(ptxpath, closest_hit_func, entry_point_index); 
    // resurrect from void* with : optix::Material mat = optix::Material::take((RTmaterial)mat_ptr);   
}
void* OCtx_create_geometry_instance(void* geo_ptr, void* mat_ptr )
{
    return OCtx_()->create_geometry_instance(geo_ptr, mat_ptr); 
    // resurrect from void* with : optix::GeometryInstance gi = optix::GeometryInstance::take((RTgeometryinstance)gi_ptr);   
}
void* OCtx_create_geometry_group(const std::vector<void*>& v_gi_ptr)
{
    return OCtx_()->create_geometry_group(v_gi_ptr); 
} 
void* OCtx_create_acceleration( const char* accel )
{
    return OCtx_()->create_acceleration( accel ); 
}
void  OCtx_set_acceleration(void* gg_ptr, void* ac_ptr )
{
    OCtx_()->set_acceleration(gg_ptr, ac_ptr); 
}
void OCtx_set_geometry_group_context_variable( const char* key, void* gg_ptr )
{
    OCtx_()->set_geometry_group_context_variable(key, gg_ptr); 
}
void OCtx_compile()
{
    OCtx_()->compile(); 
...
void OCtx_launch(unsigned entry_point_index, unsigned width, unsigned height, unsigned depth)
{
    OCtx_()->launch(entry_point_index, width, height, depth); 
}
void OCtx_launch(unsigned entry_point_index, unsigned width, unsigned height)
{
    OCtx_()->launch(entry_point_index, width, height); 
}
void OCtx_launch(unsigned entry_point_index, unsigned width)
{
    OCtx_()->launch(entry_point_index, width); 
}
