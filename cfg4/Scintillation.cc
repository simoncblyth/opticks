...
//
G4VParticleChange*
Scintillation::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)

// This routine simply calls the equivalent PostStepDoIt since all the
// necessary information resides in aStep.GetTotalEnergyDeposit()

{
        return Scintillation::PostStepDoIt(aTrack, aStep);
}
...
//
G4VParticleChange*
Scintillation::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...
void Scintillation::BuildThePhysicsTable()
{
...
void Scintillation::SetTrackSecondariesFirst(const G4bool state)
{
        fTrackSecondariesFirst = state;
}

void Scintillation::SetFiniteRiseTime(const G4bool state)
{
        fFiniteRiseTime = state;
}

void Scintillation::SetScintillationYieldFactor(const G4double yieldfactor)
{
        fYieldFactor = yieldfactor;
}

void Scintillation::SetScintillationExcitationRatio(const G4double excitationratio) {
...
void Scintillation::SetScintillationByParticleType(const G4bool scintType)
{
        if (fEmSaturation) {
           G4Exception("Scintillation::SetScintillationByParticleType", "Scint02",
...
void Scintillation::AddSaturation(G4EmSaturation* sat)
{
        fEmSaturation = sat;
}

void Scintillation::RemoveSaturation()
{
        fEmSaturation = NULL;
}
...
G4double Scintillation::GetMeanFreePath(const G4Track&,
                                          G4double ,
...
G4double Scintillation::GetMeanLifeTime(const G4Track&,
                                          G4ForceCondition* condition)
{
        *condition = Forced;
...
G4double Scintillation::sample_time(G4double tau1, G4double tau2)
{
...
GetScintillationYieldByParticleType(const G4Track &aTrack, const G4Step &aStep)
{
  ...
  if (!Scint_Yield_Vector) {
    ...
    G4Exception("Scintillation::PostStepDoIt","Scint01",
  ...
  if(PreStepKineticEnergy <= Scint_Yield_Vector->GetMaxEnergy()){
    ...
    G4ExceptionDescription ed;
    ed << "\nScintillation::GetScintillationYieldByParticleType(): Request\n"
    ...
    G4Exception("Scintillation::GetScintillationYieldByParticleType()",
  ...
  G4cout << "\n--- Scintillation::GetScintillationYieldByParticleType() ---\n"
	 <<   "--\n"
