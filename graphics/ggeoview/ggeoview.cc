...
//  /usr/local/env/graphics/glfw/glfw-3.1.1/tests/events.c
...
int main(int argc, char** argv)
{
    ...
    for (;;)
    {
        for (i = 0;  i < count;  i++)
        {
            ...
            if(i == 0)
            {
                _update_fps_counter( slots[i].window );

                int fwidth, fheight;

                glfwGetFramebufferSize(slots[i].window, &fwidth, &fheight);

                glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

                glViewport(0, 0, fwidth, fheight);

                scene.draw(fwidth, fheight);
