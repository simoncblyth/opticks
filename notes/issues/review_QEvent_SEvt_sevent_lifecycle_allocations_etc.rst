...
DONE : make the SVN commits for the updated integration
---------------------------------------------------------

1. tested build without opticks following instructions from jvi to avoid trac breakage

::

    [blyth@localhost offline]$ svn commit -m "Updated Opticks integration requiring latest Opticks, hidden behind WITH_G4CXOPTICKS macro, tests ongoing"
...
jvi::
    ...
    2910 ntds0(){ OPTICKS_MODE=0 ntds3 ; }  #0b00   Ordinary running without Opticks involved at all  
    2911 ntds1(){ OPTICKS_MODE=1 ntds3 ; }  #0b01   Running with only Opticks doing the optical propagation 
    2912 #ntds2(){ OPTICKS_MODE=2 ntds3 ; } #0b10   Geant4 only with Opticks instrumentation (that was original idea) 
    2913                                    #       but U4RecorderTest running superceeds that :  perhaps assert that this mode is not used 
    2914 ntds3()                            #0b11   Running with both Geant4 and Opticks optical propagation
    2915 {  
...
DONE : logging rationalize QEvent=INFO SEvt=INFO
-------------------------------------------------
