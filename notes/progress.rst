...
* 28th : May 25–29, 2026, Bangkok, Thailand
* 27th : Oct 19–25, 2024, Krakow, Poland
...
* progress notes continue in :doc:`../release_notes`  starting with v0.4.2 2025/05/15


2025 Apr : Gitlab CI, cvmfs release automation
------------------------------------------------

* o : 04/31 : remove the confusing Default EventMode, set actual default OPTICKS_EVENT_MODE to Minimal, increase MaxBounceDefault from 9 to 31
* o : 04/30 : qcf_ab.f90 f2py approach is more than twice as fast as numpy qcf.py approach but thats nowhere near the CPP approach used by sysrap/sseq_index.h
* o : 04/28 : Prepare to addtag.sh v0.4.1 fixing WITH_CUSTOM4 regression and PMT data access on device problem
* o : 04/23 : shakedown ctest running for release, avoid stale tests in release
* o : 04/22 : leap to CMake CUDA LANGUAGES support for multi-arch support, as explained in notes/issues/OPTIX_ERROR_INVALID_INPUT_with_optixModuleCreate_and_distributed_ptx.rst
* o : 04/21 : provide override for path to PTX source and look into how to change the compute capability target of generated PTX
* o : 04/16 : more automation for ok release making using okdist-deploy-to-cvmfs
* o : 04/16 : fix release omissions, now G4CXTest_raindrop.sh with its python and other bash works from the release
* o : 04/14 : review and annotate the C++ sseq array history comparison sseq_index_test.sh that can replaces the slow python alternative
* o : 04/10 : reproduce chep 2024 v0 fig5 from metadata, plus the Philox equiv which shows better linearity down to a few million photons


2025 Mar : getting OJ gitlab CI operational using docker image
----------------------------------------------------------------

* o : 03/17 :  try to make non-zero return codes within opticks-setup-generate created bashrc immune to set -e that gitlab-ci is using
* o : 03/12 : notes on docker expts, created junosw/cuda:12.4.1-runtime-rockylinux9 image in which junosw builds, runtimeplus for junosw+opticks in progress
* o : 03/01 : plug the leaking of QEvent::gs reported by Ilker

2025 Feb
---------

* o : 02/20 : switch OFF DEBUG_TAG from sysrap/CMakeLists.txt avoiding three QSimTest_ALL.sh fails from stagr usage
* o : 02/14 : remove qudarap-prepare-installation from opticks-prepare-installation as with Philox there is no longer need to prepare curandState files

2025 Jan : testing the migrations revealed reversions, now fixed
-------------------------------------------------------------------

* o : 01/02 : investigate ~/j/issues/jok-tds-mu-running-NOT-A-SENSOR-warnings.rst from qsim::propagate_at_surface_CustomART
