...
+---------+-----+------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------+
| v0.6.1  | 61  | 2026/03/20 | add sanity check automatically preventing okdist-- with a stale build                                                                                         |
+---------+-----+------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
* 2026-09-04 b03847371 - add shaderExecutionReordering check to SOPTIX_Properties and OPTIX_CHECK_NOTHROW
* 2026-09-04 178926466 - add okdist-scp-with-hash to allow md5sum checks of tarballs before cvmfs_server ingest
...
* 2026-03-26 992e13ce9 - avoid AddGenstep sim assert test fail, further logging control for geometry solids conversion


v0.6.2 2026/03/25 : improved conversion logging, notes on SProf.sh profiling
-------------------------------------------------------------------------------

* 2026-03-25 5167a7976 - improve sn, U4Solid, U4Polycone logging control, add notes on SProf.sh profiling


v0.6.1 2026/03/20 : add sanity check automatically preventing okdist-- with a stale build
-------------------------------------------------------------------------------------------

* 2026-03-20 8dcae61d2 - add okdist-stamp-delta okdist-stamp check for stale build relative to source prior to minting a distribution
...
* 11/19 experiment with async CUDA calls for hitlite merging in SPM.cu and QEvt.cc using NP_future SPM_future that callers need to wait on
* 11/18 investigate async CUDA streams in SPM_test.sh, bottom up async approach has advantage of being easy to test, its all about deferring the sync as far as can
* 11/14 integrate sysrap/SPM.cu per-launch CUDA/Thrust hitlite merging and add hitlitemerged array handling, also add NumPy merge verification to sysrap/sphotonlite.py
* 11/13 standalone test of CUDA Thrust impl of sphotonlite hit merging generated over many iterations with Grok
* 11/12 rename qudarap/QEvent.hh to qudarap/QEvt.hh to avoid name clash when using Geant4 with Qt viz driver that Nicola reports caused some vtable related crash
* 11/04 prep for using hitlite from OJ for pmt_hit_type:2 aka muon hits
* 11/03 integrate photonlite, hitlite into the SEvt SEventConfig workflow, add hitlocal (CPU derived from hit) to simplify validation of hitlite, which effectively does locatization on GPU
* 10/31 rejig squad.h quad2/prd packing to make room for lposfphi, to be persisted to a future sphotonlite that is a quarter the size of the sphoton
