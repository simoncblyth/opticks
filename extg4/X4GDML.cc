...
G4VPhysicalVolume* X4GDML::Parse(const char* path, BMeta** meta) // static 
{
    X4GDML cg ;
    cg.read(path);  
    *meta = cg.getMeta();
...
G4VPhysicalVolume* X4GDML::Parse(const char* dir, const char* name) // static 
{
    std::string path = BFile::FormPath(dir, name);
    return X4GDML::Parse(path.c_str());
}
G4VPhysicalVolume* X4GDML::Parse(const char* dir, const char* name, BMeta** meta) // static 
{
    std::string path = BFile::FormPath(dir, name);
    return X4GDML::Parse(path.c_str(), meta);
}
...
G4VPhysicalVolume* X4GDML::getWorldVolume() const 
{
...
But enough to be useful. eg::
    ...
    epsilon:cfg4 blyth$ X4GDMLTest /tmp/v1.gdml
    ...
    2020-12-08 18:48:40.448 INFO  [3590231] [*X4GDML::getUserMeta@245] auxlist 0x7ff7e9726438
    2020-12-08 18:48:40.449 INFO  [3590231] [BMeta::dump@199] X4GDMLTest::test_Parse
    {
        "lvmeta": {
...
void X4GDML::addLVMeta(const BMeta* lvmeta)
{
    LOG(info) << " NOT IMPLEMENTED " << lvmeta ; 
}


const char* X4GDML::LVMETA = "lvmeta" ; 
const char* X4GDML::USERMETA = "usermeta" ; 

BMeta* X4GDML::getMeta() const 
{
...
void X4GDML::addMeta(const BMeta* meta)
{
...
void X4GDML::dumpLVMeta(const char* msg) const
{
    const G4GDMLAuxMapType* auxmap = m_parser->GetAuxMap();
    LOG(info) << msg 
...
void X4GDML::dumpUserMeta(const char* msg) const
{
    LOG(LEVEL) << msg ; 
...
BMeta* X4GDML::getUserMeta() const 
{
    const G4GDMLAuxListType* auxlist = m_parser->GetAuxList() ; 
...
void X4GDML::addUserMeta(const BMeta* user)
{
...
void X4GDML::Export(const char* dir, const char* name, const G4VPhysicalVolume* const world, const BMeta* meta)
{
    std::string path = BFile::FormPath(dir, name);
    X4GDML::Export( path.c_str(), world, meta ); 
}

void X4GDML::Export(const char* path, const G4VPhysicalVolume* const world, const BMeta* meta)
{
    assert( world );
    X4GDML cg ; 
...
void X4GDML::write( const char* path,  const G4VPhysicalVolume* const world, const BMeta* meta )
{
...
std::string X4GDML::GenerateName(const char* name, const void* const ptr, bool addPointerToName )
{
