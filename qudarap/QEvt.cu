...
__global__ void _QEvt_checkEvt(sevent* evt, unsigned width, unsigned height)
{
    ...
    printf("//_QEvt_checkEvt width %d height %d photon_id %3d genstep_id %3d  gs.q0.i ( %3d %3d %3d %3d )  gencode %d num_photon %d \n",
       width,
       height,
       photon_id,
       genstep_id,
...
{
    printf("//QEvt_checkEvt width %d height %d \n", width, height );
    _QEvt_checkEvt<<<numBlocks,threadsPerBlock>>>( evt, width, height  );
}

/**
QEvt_count_genstep_photons
-------------------------------
...
#ifdef DEBUG_QEVENT
    printf("//QEvt_count_genstep_photons sevent::genstep_numphoton_offset %d  sevent::genstep_itemsize  %d  \n",
...
/**
QEvt_count_genstep_photons_and_fill_seed_buffer
---------------------------------------------------

This function does the same as the above two functions.
It is invoked from QEvt::setGenstep
...
#ifdef DEBUG_QEVENT
    printf("//[QEvt_count_genstep_photons_and_fill_seed_buffer iexpand \n" );
#endif
