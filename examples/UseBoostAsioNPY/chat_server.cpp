//  /usr/local/env/boost/basio/example/cpp03/chat/chat_server.cpp
//
// chat_server.cpp
// ~~~~~~~~~~~~~~~
//
...
chat_session::chat_session(boost::asio::io_context& io_context, chat_room& room)
    : 
    socket_(io_context),
    room_(room)
{
...
{
    std::cout << "chat_session::handle_read_header" << std::endl ; 
...
{
    std::cout << "chat_session::handle_read_body" << std::endl ; 
    if (!error)
    {
...
{
    std::cout << "chat_server::chat_server" << std::endl ; 
    start_accept();
}

void chat_server::start_accept()
{
    std::cout << "chat_server::start_accept" << std::endl ; 
...
{
    std::cout << "chat_server::handle_accept" << std::endl ; 
    if(!error)
    {
        session->start();
    }
