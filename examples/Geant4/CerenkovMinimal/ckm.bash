...
ckm-gentest
    loads the direct gensteps from the geocache keydir identified by 
    the OPTICKS_KEY envvar and generates photons from them.  
    These photons are saved into KEYDIR/tests/CCerenkovGeneratorTest/so.npy 
    where KEYDIR is internally resolved into the geocache keydir 
    (aka idpath) of the geometry.

    As shown below these photons from gensteps closely match those generated by 
    G4 normally and also by Opticks on GPU.  This matching is the 
    justification for the two-executable debugging approach of having the
    first minimally instrumented example executable paired with the 
...
with the gentest ones, max deviation 6e-5::
