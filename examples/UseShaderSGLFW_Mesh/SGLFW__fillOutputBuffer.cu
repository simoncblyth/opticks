...
__global__ void _SGLFW__fillOutputBuffer( uchar4* output_buffer, int32_t width, int32_t height )
{
...
{
    printf("//SGLFW__fillOutputBuffer width %d height %d \n", width, height );
    _SGLFW__fillOutputBuffer<<<numBlocks,threadsPerBlock>>>( d_output_buffer, width, height );
}
