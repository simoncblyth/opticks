...
opticks-setup-generate(){
    ...
    echo "# $path generated by $FUNCNAME" > $path 
    opticks-setup-hdr-      >> $path
    opticks-setup-funcs-    >> $path
...
if [ "\$BASH_SOURCE" == "\$0" ]; then
   echo ERROR the \$BASH_SOURCE file needs to be sourced not executed
   exit 1
fi 

if [ "\$(which nvcc)" == "" ]; then 
   echo ERROR the original PATH must have nvcc in it to locate CUDA 
   exit 1
fi  

if [ -z "\$OPTICKS_OPTIX_PREFIX" ]; then
   echo ERROR missing required input envvar OPTICKS_OPTIX_PREFIX 
   exit 1
fi 
...
opticks-cuda-prefix(){
  : having nvcc in original PATH is required to identify the CUDA prefix
  local nvcc=$(which nvcc)
  if [ -n "$nvcc" ]; then 
      echo $(dirname $(dirname $nvcc))
  fi   
}

opticks-setup-funcs-(){ 
   echo "# $FUNCNAME"
   type opticks-cuda-prefix
   type opticks-setup- 
...
export OPTICKS_CUDA_PREFIX=\$(opticks-cuda-prefix)
